...
enum state GameState::checkStatus(Player *player)
{
  for (int i = 0; i < 3; i++)
  {
    for (int k = 0; k < 3; k++)
  ...
  if (player->getArchetype() == "swarm")
  {
    if (board->getMark(0, 0) == board->getMark(0, 2) && board->getMark(0, 0) == board->getMark(2, 0) && board->getMark(0, 0) == board->getMark(2, 2))
    {
      winner = board->getMark(0, 0);
      return WON;
    }
  }
